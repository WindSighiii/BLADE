...
def data_partition(dataset_name):
    ...
    interaction_count =0
    if dataset_name[:3] == 'QKV' or dataset_name[:3] == 'QKA' or dataset_name[:3] =='KUA':
    ...
    item_id_mapping = {old_id: new_id for new_id, old_id in enumerate(sorted(itemset), 1)}
      # 重新映射所有数据集中的物品ID
    for user_data in [user_train, user_valid, user_bvt, user_test]:
        for user, items in user_data.items():
            for i in range(len(items)):
                old_item_id = items[i][0]
                new_item_id = item_id_mapping[old_item_id]
                items[i][0] = new_item_id

    itemset = set(item_id_mapping.values())
    ...
    print_dataset_msg += set_color('# Interaction Num: ' + str(interaction_count) + '\n', 'white')
