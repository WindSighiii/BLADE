...
class MLP(nn.Module):
    ...
    def forward(self,inputs):
        ...
        x = self.fc2(x)
        return x
def computeMask(behaviors):
    is_click = (behaviors[:, :, 0] == 1)  # [B, T], bool
    # 检查是否“仅包含点击”（即整行只有一个 1）
    is_only_click = (behaviors.sum(dim=-1) == 1)  # [B, T], bool

    # 同时满足两个条件才保留
    click_only_mask = (is_click & is_only_click).float()  # [B, T], float mask (1.0 or 0.0)
    return click_only_mask
class FrequencyLayer(nn.Module):
    def __init__(self, config):
        super(FrequencyLayer, self).__init__()
        self.out_dropout = nn.Dropout(config['frequency_dropout'])
        self.LayerNorm = nn.LayerNorm(config['hidden_dims'], eps=1e-12)
        self.c = config['maxlen'] // 2 + 1  # rFFT输出频率数

        self.beta = nn.Parameter(torch.randn(1, 1, 1))  # 可学习高频权重

    def forward(self, input_tensor, mask=None):
        """
        input_tensor: [B, T, D]
        mask: [B, T] (optional), 1 表示该位置启用频域滤波，0 表示保留原始输入
        """
        B, T, D = input_tensor.shape

        # 频域变换
        x_freq = torch.fft.rfft(input_tensor, dim=1, norm='ortho')  # [B, T//2+1, D]

        # 低通滤波
        low_pass = x_freq.clone()
        low_pass[:, self.c:, :] = 0  # 只保留前 c 个频率

        # 逆变换回时域
        low_pass_time = torch.fft.irfft(low_pass, n=T, dim=1, norm='ortho')  # [B, T, D]

        # 高频部分
        high_pass_time = input_tensor - low_pass_time

        # 滤波后的表示（调节高频部分）
        filtered = low_pass_time + (self.beta ** 2) * high_pass_time  # 保证权重非负

        # 若提供 mask，则选择性替换
        if mask is not None:
            mask = mask.unsqueeze(-1).to(dtype=filtered.dtype)  # [B, T, 1]
            output = mask * filtered + (1 - mask) * input_tensor
        else:
            output = filtered
